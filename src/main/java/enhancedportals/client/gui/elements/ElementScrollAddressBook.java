...
import enhancedportals.client.gui.GuiAddressBook;
...
public class ElementScrollAddressBook extends BaseElement
{
    ...
    public ElementScrollAddressBook(GuiAddressBook gui, ItemAddressBook b, int x, int y)
    {
    ...
    public boolean handleMouseClicked(int x, int y, int mouseButton)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            if (mouseOverSmall)
            {
                if (GuiScreen.isShiftKeyDown())
                {
                    ((GuiAddressBook) parent).onEntryDeleted(scrollAmount + i);
                    break;
                }
                else
                {
                    ((GuiAddressBook) parent).onEntryEdited(scrollAmount + i);
                    break;
                }
            }
        }
    ...
    protected void drawContent()
    {
        ...
        if (addressBook.glyphList.size() >= 6)
        {
            canScroll = true;
            handleMouse();
        }
