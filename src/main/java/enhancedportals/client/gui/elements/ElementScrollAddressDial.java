...
import enhancedportals.client.gui.GuiAddressDial;
...
public class ElementScrollAddressDial extends BaseElement
{
    private TileDialingDevice dial;
    private ItemAddressBook addressBook;
    private float currentScroll = 0f;
    private boolean isScrolling = false, wasClicking = false;
    private int scrollAmount = 0;
    private int offsetX = 5, offsetY = 5, sizeMButton = 196, sizeSButton = 20, buttonSpacing = 2, entryHeight = 22;


    public ElementScrollAddressDial(GuiAddressDial gui, TileDialingDevice d, ItemAddressBook b, int x, int y)
    {
        super(gui, x, y, gui.getSizeX() - 38, gui.getSizeY() - 115);
        LogHelper.debug("Gui get X: " + gui.getSizeX() + "Gui Get Y: " + gui.getSizeY());
        dial = d;
        addressBook = b;
    ...
    public void addTooltip(List<String> list)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            if (scrollAmount + i >= addressBook.glyphList.size())
            {
                break;
            }
    ...
    public boolean handleMouseClicked(int x, int y, int mouseButton)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            GlyphElement ea = addressBook.glyphList.get(scrollAmount + i);
            GlyphElement ed = dial.glyphList.get(scrollAmount + i);
            int entryOffset = i * entryHeight;
            boolean mouseOverEntry = y >= posY + offsetY + entryOffset && y <= posY + offsetY + entryOffset + 20,
            ...
            /*if (mouseOverSmall)
            {
                if (GuiScreen.isShiftKeyDown())
                {
                    ((GuiAddressDial) parent).onEntryDeleted(scrollAmount + i);
                    break;
                }
                else
                {
                    ((GuiAddressDial) parent).onEntryEdited(scrollAmount + i);
                    break;
                }
            }*/
        }
    ...
    protected void handleMouse()
    {
        ...
        if (isScrolling)
        {
            ...
            int items = addressBook.glyphList.size() - 5 + 1;
            ...
            int max = addressBook.glyphList.size() - 5;
    ...
    protected void drawContent()
    {
        ...
        if (addressBook.glyphList.size() >= 3)
        {
            canScroll = true;
            handleMouse();
        }
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            GL11.glColor3f(1f, 1f, 1f);
            GlyphElement e = addressBook.glyphList.get(scrollAmount + i);
            ...
            boolean mouseOverEntry = mouseY >= posY + offsetY + entryOffset && mouseY <= posY + offsetY + entryOffset + 20,
                    ...
                    mouseOverSmall = mouseOverEntry && mouseX >= posX + offsetX + sizeMButton + buttonSpacing && mouseX < posX + offsetX + sizeMButton + buttonSpacing + sizeSButton, delete = GuiScreen.isShiftKeyDown();
